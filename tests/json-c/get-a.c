...
int main(void)
{
  ...
  TRY(p = csonpath_set_path(p, "$.z|$.b.B"));
  ret = csonpath_find_first(p, jobj);
  assert(ret);
  assert(!strcmp(json_object_get_string(ret), "la y'a l'B"));

  TRY(p = csonpath_set_path(p, "$.z.a|$.b.B"));
  ret = csonpath_find_first(p, jobj);
  assert(ret);
  assert(!strcmp(json_object_get_string(ret), "la y'a l'B"));

  TRY(p = csonpath_set_path(p, "$[0]|$.b.B"));
  ret = csonpath_find_first(p, jobj);
  assert(ret);
  assert(!strcmp(json_object_get_string(ret), "la y'a l'B"));

  TRY(p = csonpath_set_path(p, "$.b.B|$.z"));
  ret = csonpath_find_first(p, jobj);
  assert(ret);
  assert(!strcmp(json_object_get_string(ret), "la y'a l'B"));

  TRY(p = csonpath_set_path(p, "$..B"));
  ret = csonpath_find_first(p, jobj);
  assert(ret);
  assert(!strcmp(json_object_get_string(ret), "la y'a l'B"));
  ...
  TRY(p = csonpath_set_path(p, "$.c.B"));
  ret = csonpath_find_first(p, jobj);
  assert(!ret);

  TRY(p = csonpath_set_path(p, "$.array[1]"));
  ret = csonpath_find_first(p, jobj);
  assert(ret);
  ...
  TRY(p = csonpath_set_path(p, "$.array[*]"));
  ret = csonpath_find_all(p, jobj);
  assert(ret);
